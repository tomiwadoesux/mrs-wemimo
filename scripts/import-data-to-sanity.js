...
if (!projectId || !dataset || !token) {
  ...
  console.error('Ensure NEXT_PUBLIC_SANITY_PROJECT_ID, NEXT_PUBLIC_SANITY_DATASET, and SANITY_API_TOKEN are set in your .env.local file.')
  console.error('\nTo generate a token:')
...
async function deleteAll(type) {
  ...
  try {
    ...
    console.log(`🗑️ Deleting ${docs.length} existing documents of type "${type}"...`)
    const transaction = client.transaction()
    docs.forEach((doc) => {
      transaction.delete(doc._id)
    })
    await transaction.commit()
    console.log(`✅ Successfully deleted ${docs.length} "${type}" documents.\n`)
    return docs.length
  } catch (error) {
  ...
  try {
    ...
    for (const doc of testimonialDocs) {
      try {
        ...
        count++
        process.stdout.write(`\r✅ Imported ${count}/${testimonialDocs.length} testimonials`)
      } catch (error) {
        console.error(`\n❌ Error importing testimonial "${doc.name}":`, error.message)
      }
    }
...
async function importGallery() {
  ...
  for (const image of galleryImages) {
    ...
    try {
      ...
      await client.create(doc)
      count++
      process.stdout.write(`\r✅ Uploaded and imported ${count}/${galleryImages.length} gallery items`)
    } catch (error) {
      console.error(`\n❌ Error importing gallery item for "${image.src}":`, error.message)
    }
  }
...
async function main() {
  try {
    ...
    console.log('3. You can add descriptions or re-organize them in the Studio.')
    console.log('\n')
  } catch (error) {
